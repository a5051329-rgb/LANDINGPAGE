...
(function(){
  ...
  flow.forEach((id,i)=>{
    ...
    if(!sec||!next) return;
    if(sec.querySelector(':scope > .section-down')) return;
...
(function(){
  ...
  function openImage(slide){
    ...
    const img=slide.querySelector('img');
    const no=slide.querySelector('.float-copy span')?.textContent?.trim() || '';
    const title=slide.querySelector('.float-copy b')?.textContent?.trim() || img.alt || 'Mirafeel 實際穿著情境';
    const sub=slide.querySelector('.float-copy p')?.textContent?.trim() || '';
    modalImg.src=img.src;
    modalImg.alt=img.alt || title;
    modalCap.innerHTML=`<div class="modal-kicker">Mirafeel Point ${no}</div><h3 class="modal-title">${title}</h3><p class="modal-sub">${sub}</p>`;
    modal.classList.add('open');
    modal.setAttribute('aria-hidden','false');
    stop();
  }
